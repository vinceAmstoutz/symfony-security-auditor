...
package type), built on the framework-agnostic
`vinceamstoutz/security-auditor-core` package that lives in `packages/core/` of
this monorepo. Uses a dual-agent attacker/reviewer loop backed by `symfony/ai`
...
| Static analysis   | PHPStan max + phpstan-strict-rules + custom rules (`FinalRule`, `MaxParameterCountRule`, `NoEmptyCatchRule`, `NoSilencingErrorHandlerRule`, `ForbiddenTestAttributeRule`, `SprintfOverConcatRule`, `OversizedDocblockRule` (docblock prose capped at 8 lines, annotations and their continuation lines exempt — the `.claude/rules/no-comments.md` gate) — in `tools/PHPStan/`) + symplify/spaze rules + Rector                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Layer conformance | deptrac (the `packages/core` ↔ bundle package boundary + the DDD layer rules inside the core — `deptrac.yaml`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Score detection quality | `bin/castor eval` (audits a ground-truth fixture, reports precision/recall, and fails on any drift from a recorded `--write-baseline`) |
...
```text
packages/core/src/    # vinceamstoutz/security-auditor-core — framework-agnostic
  Audit/
    Domain/          # Pure PHP — no framework, no I/O
      ...
      Model/         # Value objects and enums (AnthropicOptionDialect (which model ids accept the max_tokens/response_format option dialect), Vulnerability [+ `of()` factory + CodeLocation/VulnerabilityClassification/VulnerabilityNarrative], SymfonyMapping [+ `of()` + ProjectFileInventory/AccessControlMap], AuditReport [+ ReportIdentity], ExecutiveSummary (stakeholder view: risk level + severity/type/file distributions), ProjectFile, ProjectFileType [+ `archetype()`], SurfaceArchetype (framework-neutral file shape; ProjectFileType carries both the Symfony and the Laravel vocabulary, each case mapped onto one), FrameworkVocabulary (framework name/template language/idiomatic fixes for the synthesizer prompts), ChunkingVocabulary (the profile's surface priority, entrypoint feature suffixes and template extensions, read by FileChunker), CvssEstimate (heuristic CVSS v4.0 per finding), EntrypointAccessControl, AuthorizationRuleCapability, FormBinding, TokenUsageSnapshot, VulnerabilityHydrationResult, VulnerabilityDropReason, ReviewerFeedback/AcceptedFindingFeedback, …) — public factories use `of()`; the wide `create()` factories were removed in 2.0
      ...
      Port/          # Cross-layer ports — BC-covered (LLMClientInterface, LLMResponse, ProjectFileScannerInterface, AdvisoryDatabaseInterface, SecretScrubberInterface, TokenEstimatorInterface, PricingProviderInterface [one cache-aware port since 2.0], RateLimiterInterface, ProgressReporterInterface, StaticPreScannerInterface, CodeSlicerInterface, EntrypointAccessControlParserInterface, AuthorizationRuleParserInterface, FormBindingParserInterface, AccessControlConfigParserInterface, ProjectFileTypeClassifierInterface, GitChangedFilesResolverInterface, ReviewerFeedbackProviderInterface, TriageMemoryRecorderInterface, AttackerSkillPromptRendererInterface) and `@internal` seams (BatchCapableLLMClientInterface, ToolBatchCapableLLMClientInterface — both taking LLMRequest/ToolLLMRequest value objects, *PromptBuilderInterface, AttackerCacheInterface, ContextAwareAttackerCacheInterface, ReviewerCacheInterface, ReviewerFeedbackSnapshotInterface) + null-object port defaults (NullStaticPreScanner, NullReviewerFeedbackProvider, NullCodeSlicer, NullEntrypointAccessControlParser, NullAuthorizationRuleParser, NullFormBindingParser, NullAccessControlConfigParser, NullProgressReporter — all `@internal`)
    ...
    Infrastructure/  # I/O adapters
      ...
      Scan/          # Framework-neutral scanners — RegexStaticPreScanner, SarifImportingPreScanner (merges scan.import_sarif SARIF results as risk markers), RegexCodeSlicer + its line-retention machinery (SecurityRelevantLineClassifier, BlockCommentStripper, StringLiteralMasker, HeredocLineTracker, ParenContinuationTracker, ThisCallReachability)
      Diff/          # ProcessGitChangedFilesResolver (git diff for --since)
      Settings/      # The binary's own config file — XdgConfigPathResolver, StandaloneConfig(+Factory/Loader), StandalonePlatformConfig(+Resolver), YamlStandaloneConfigWriter, OfflineOnlyPlatformGuard + Exception/*
  ...
  Command/           # AuditCommand (Symfony Console: audit:run, alias audit) + AuditCommandInput, AuditPresenter, ConsoleBanner (the identity banner, also rendered by StandaloneApplication for every non-audit command), ReportWriter, AuditExitCodeResolver, ExitCode enum (0 pass / 1 gate tripped / 2 budget aborted / 3 audit never completed), AuditFailureExitCodeListener (maps a console.error on audit:run to exit 3), AuditCommandHelp, OutputFormat enum (console|executive|json|sarif|html|markdown|junit|github|github-comment), Baseline (accepted-finding suppression); DiffCommand (audit:diff — compares two JSON reports by finding fingerprint) + ReportDiffer, ReportDiff/DiffFinding, DiffPresenter, DiffOutputFormat enum (console|json); TrendCommand (audit:trend — tracks finding counts across two or more JSON reports) + ReportTrendAnalyzer, ReportTrend/TrendPoint, TrendPresenter, TrendOutputFormat enum (console|json)
src/                   # vinceamstoutz/symfony-security-auditor — the Symfony bundle
  Audit/Infrastructure/
    Config/            # Symfony DI wiring — CoreCompositionRoot (takes one FrameworkProfileInterface), HostCompositionRootLoader, CompositionRootLoader, ContainerParameterRegistrar, PromptVersions, AuditConfigurationDefinition/Processor, Attacker/LlmClient definition factories, SymfonyProfile, Registrar/{core registrars, Symfony/* profile registrars}
    Prompt/            # Symfony prompt content — AttackerPromptBuilder (+ SymfonyMappingContextRenderer, NumberedFileContextRenderer, Skill/{one *AttackerSkill per attack surface, SymfonySkillSet enumerating them}), ReviewerPromptBuilder (+ Reviewer/*)
    Scan/              # Symfony source parsers — SymfonyProjectFileTypeClassifier, SymfonyChunkingVocabulary, SymfonyYamlSecurityConfigParser, PhpParser{ControllerAccessControl,VoterCapability,FormBinding}Parser, Route/IsGranted attribute parsers
  Standalone/          # StandaloneApplication(Factory), StandaloneContainerFactory, BundleExtensionLoader, StandaloneConsoleCommandFactory
...
`AdvisoryDatabaseInterface` — the rest of `packages/core/src/Audit/Domain/Port/`
is `@internal` since 2.0), Domain models/enums/exceptions, `RunAuditUseCase`,
...
`@internal` tag. A new interface under `packages/core/src/Audit/Domain/Port/` is
`@internal` **by default**; joining the BC list is a deliberate step, documented
in `docs/versioning.md` and `docs/extending.md` — see
...
- [`domain-models.md`](.claude/rules/domain-models.md) — immutability,
  copy-on-write, deterministic IDs in `packages/core/src/Audit/Domain/**`.
