...
- Vulnerability `id` is deterministic: `VULN-{sha1(type+filePath+lineStart)[0..7]}`
  (no microtime, no title) — do not change this scheme.
- Adding a `VulnerabilityType` case requires updating `category()`,
  `owaspReference()`, `owaspReferenceUrl()`, and `cwe()` — nothing else
  changes.
