...
- `Command\AuditCommandInput` — Symfony Console MapInput requires public mutable properties with property-level defaults; promoted readonly constructor params are invisible to its reflection.

Each opt-out site declares the reason in a leading code comment and cites this rule. Anything outside that list must be `final readonly`. If inheritance feels needed, introduce an interface instead.
...
When you touch a file that bundles responsibilities, extract them into new classes rather than adding more.
...
Built-in throwables that are **not** authored by us (`\JsonException`, `\ValueError`, `\TypeError`, `\Throwable` in broad catches) are fine to consume — the rule applies to what we *throw*, not what we *catch*.
