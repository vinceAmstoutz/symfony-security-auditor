...
- **`--dry-run` now warns when PoC or fix synthesis is enabled**, since neither
  ...
  `FixSynthesizer` each make their own LLM call per qualifying finding — a
  cost that can't be known before the attacker has actually run and found
  something — so `EstimateAuditCostUseCase`'s cost breakdown has no line item
  for either. `AuditPresenter::synthesisCostWarnings()` prints a stderr
  warning naming the enabled stage(s) (`audit.poc_synthesis.enabled` /
  ...
  profile turns PoC synthesis on by default, making it the case most likely
  to hit this gap.
