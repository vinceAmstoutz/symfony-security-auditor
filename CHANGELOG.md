...
- **`--dry-run` now also counts tool round-trip overhead.** With
  ...
  the option that actually bounds how many tool rounds a chunk may take,
  against the `CALIBRATION_MAX_TOOL_ITERATIONS` bound the 50% was measured at:
  halving the bound to 4 charges 25% instead of 50%, and raising it to 16
  charges 100%. Lowering `max_tool_iterations` to cut cost is now reflected in
  the estimate rather than ignored by it.
