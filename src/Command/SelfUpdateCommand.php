...
final readonly class SelfUpdateCommand
{
    ...
    private function report(SymfonyStyle $symfonyStyle, SelfUpdateResult $selfUpdateResult): int
    {
        ...
        if (PricingCatalogRefreshOutcome::Failed === $selfUpdateResult->pricingCatalogRefresh) {
            $symfonyStyle->warning('Could not refresh the bundled pricing catalog. Cost figures keep using the catalog already in place — the last successful refresh if there was one, otherwise the one frozen into the binary at build time. The next update that installs a new binary refreshes it again.');
        }
