...
final readonly class TwigExtensionAttackerSkill implements AttackerSkillInterface
{
    ...
    public function block(): string
    {
        return <<<'SKILL'
            ...
            - Functions/filters returning raw HTML (string concatenation, `sprintf` into markup) that declare `is_safe: ['html']` without sanitizing the input first — Twig auto-escapes a filter/function's output unless `is_safe: ['html']` is declared, so an author who adds it just to stop visibly double-encoded markup, without sanitizing untrusted content first, disables escaping entirely and ships stored/reflected XSS.
