...
/** @internal not part of the BC promise — the enum *values* (`pipeline.started`, `stage.started`, `stage.completed`, `pipeline.completed`, `audit.started`, `audit.iteration.started`, `attacker.chunk.started`, `attacker.finding.recorded`, `review.started`, `review.completed`) are the wire-format event names carried through `ProgressReporterInterface::report()` and are stable, but the PHP enum itself is for internal use only. */
