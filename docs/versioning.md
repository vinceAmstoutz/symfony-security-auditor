...
- The composite action defined by `action.yml` at the repository root and its
  ...
  `generate-baseline`, `since`, `fail-on`, `update-badge`, `badge-path`,
  `extra-args`, `php-version`, `setup-php`, `install-dependencies`,
  `working-directory`, and its output names: `exit-code`, `report-path`,
  `findings-count`, `highest-severity`, `grade`, `badge-path`. New
