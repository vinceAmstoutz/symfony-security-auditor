...
`audit.tools_enabled` and the collection mode. With tools enabled, the
attacker can call `read_file`, `grep`, `list_files`, and `lookup_advisory`
for cross-file investigation, bounded by `audit.max_tool_iterations` — in
the default structured-collection mode those investigation tools ride
alongside each chunk's `record_vulnerability` registry. JSON output is parsed via
