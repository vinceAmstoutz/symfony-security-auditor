...
| `audit.history.enabled`                       | `bool`                                              | `false`                                               | Persist per-project finding fingerprints and, on the next run, tag findings `new` / `still_present` and report a `fixed` count. Fingerprints survive line-number drift. Essential for CI tracking remediation across PRs.                                                                                                                                                                        |
| `audit.history.dir`                           | `string`                                            | `%kernel.cache_dir%/symfony_security_auditor/history` | Directory for per-project fingerprint snapshots. Point at a persistent CI cache (or commit it) to track findings across runs.                                                                                                                                                                                                                                                                    |
