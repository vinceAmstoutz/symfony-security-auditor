...
| `max_output_tokens`          | `int` (≥ 1) | `4096`              | Maximum output tokens per LLM call, set as `max_tokens` on every platform request **to an Anthropic-dialect model**. Default `4096` — `symfony/ai`'s Anthropic bridge would otherwise apply its own much smaller default (~1000) and silently truncate `record_vulnerability` tool-call arguments mid-finding. **Only the Anthropic-dialect bridges accept the option**; `symfony/ai`'s Gemini and OpenAI Responses bridges reject the `max_tokens` key outright. Since 2.0 the dialect is decided by anchored model-id prefix (`claude-`, `claude.`, `anthropic.`, `us./eu./apac.anthropic.`) rather than a bare substring match, and a value other than the default configured against any other model is reported as a pre-flight notice instead of being dropped in silence. |
