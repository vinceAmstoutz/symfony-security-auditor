...
| `model`                      | string      | `'claude-opus-5'` | Model name used for both Attacker and Reviewer roles                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `max_output_tokens`          | `int` (≥ 1) | `8192`            | Maximum output tokens per LLM call, set as `max_tokens` on every platform request **to an Anthropic-dialect model**. Default `8192` — `symfony/ai`'s Anthropic bridge would otherwise apply its own much smaller default (~1000) and silently truncate `record_vulnerability` tool-call arguments mid-finding, and on a current Claude model the cap covers thinking and response text together, so the pre-2.0 `4096` left less room for a finding than it looks. **Only the Anthropic-dialect bridges accept the option**; `symfony/ai`'s Gemini and OpenAI Responses bridges reject the `max_tokens` key outright. Since 2.0 the dialect is decided by anchored model-id prefix (`claude-`, `claude.`, `anthropic.`, `us./eu./apac.anthropic.`) rather than a bare substring match, and a value other than the default configured against any other model is reported as a pre-flight notice instead of being dropped in silence. |
| `attacker_max_output_tokens` | `int` (≥ 1) | `null`            | Override: dedicated max output tokens for the Attacker. Falls back to `max_output_tokens` when `null`. Useful for headroom on detailed tool-call arguments. Same Anthropic-dialect caveat as `max_output_tokens` above, reported per role.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `reviewer_max_output_tokens` | `int` (≥ 1) | `null`            | Override: dedicated max output tokens for the Reviewer. Falls back to `max_output_tokens` when `null`. Same Anthropic-dialect caveat as `max_output_tokens` above, reported per role.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `provider_json_mode`         | bool        | `false`           | Send `response_format: {type: json_object}` on every LLM call **to an Anthropic-dialect model** so the provider enforces JSON output natively. **A no-op on every other dialect**, for the same bridge-compatibility reason as `max_output_tokens` above, and decided by the same anchored prefix match. Default `false`. The prompt contract (_"Return ONLY the JSON array"_) remains authoritative.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`requests_per_minute` does. For example, with the default `8192` cap and an 80
000 OTPM ceiling the limiter trips after ~9 calls/min; doubling the cap halves
that.
...
| `audit.fail_on`                               | `safe` \| `low` \| `medium` \| `high` \| `critical` | `high`    | Minimum **aggregate** risk level that makes `audit:run` exit `1` (the CI gate). The audit exits `1` when the report's risk level is at or above this threshold, `0` otherwise (a budget abort still exits `2`). _Changed in 2.0:_ the default was `critical` through 1.x, so a HIGH-risk audit used to pass; it now fails. Set `medium` / `low` to fail pull requests earlier, `critical` to restore the 1.x behaviour, or `safe` to fail on every completed audit. The `--fail-on` CLI option overrides this per run.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
its default (`8192`) bypasses `symfony/ai`'s much smaller built-in default
...
(or, under `--no-interaction`, to its default — `anthropic`, `claude-opus-5`,
...
| `--fail-on`           |       | `high`    | Minimum aggregate risk level (`safe`, `low`, `medium`, `high`, `critical`) that makes the command exit `1`. Overrides the `audit.fail_on` config key for this run. Defaults to the configured value (`high`) when omitted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `3`  | _Since 2.0._ The audit never produced a verdict: an invalid `project-path`, an option value the console rejects, conflicting options, an LLM provider abort, an unhandled exception, or **a scan that discovered no file to audit**. Check stderr. An abort after findings were already validated, and an empty scan, still write their report; the other causes write nothing, so do not assume a report file exists on a `3` |
...
there the scan did find files, and none of them changed. `--dry-run`,
`--show-scanned` and `--generate-baseline` return before the gate is computed,
so they exit `0` on an empty scan too.
