...
| `audit.triage_memory`                         | `bool`                                              | `false`    | When `true`, every finding the reviewer rejects with a non-empty `reviewer_notes` explanation is persisted to a cross-run memory file (`<cache.dir>/triage-memory.json`, keyed by type+file+title, capped at 500 entries) and surfaced back to the reviewer on later runs exactly like a baseline entry's `reason` — but recorded automatically from the reviewer's own reasoning instead of hand-curated. Merges with any baseline-sourced feedback; reviewer-verdict cache keys incorporate the combined feedback, so a newly recorded reason re-reviews affected findings. Default `false` — opt-in, since it changes reviewer prompt content and writes to disk on every run.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
