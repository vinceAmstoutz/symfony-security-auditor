...
| Key                                         | Type        | Default                                              | Description                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------- | ----------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scan.included_paths`                       | `string[]`  | `['src', 'config', 'templates', 'public/index.php']` | Project-relative directories and files that define the scan surface. Defaults match the Symfony Flex skeleton. Anything outside this list is silently skipped — ad-hoc root-level scripts, `bin/`, custom `app/` or `lib/` trees, and the build artefacts under `var/`, `public/build`, `vendor/`. Override for non-standard layouts; the audit only inspects what is listed here.                                  |
| `scan.excluded_dirs`                        | `string[]`  | `[]`                                                 | Extra directories to skip _within_ each included path. **Appended** to the hard defaults (`vendor`, `node_modules`, `.git`, `.github`, `.idea`, `.vscode`, `var/cache`, `var/log`, `public/bundles`, `public/build`, `tests`, `Tests`, `migrations`, `Migrations`, `translations`, `build`, `coverage`); never replaces them. Use this to prune sub-trees (e.g. `src/Migrations`) without rewriting the allow-list. |
