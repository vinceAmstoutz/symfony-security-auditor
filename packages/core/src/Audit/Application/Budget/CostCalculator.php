...
namespace VinceAmstoutz\SecurityAuditor\Audit\Application\Budget;
 ...
 *
 * Prompt-cache traffic is priced from the provider's own per-model cache
 * rates — every {@see PricingProviderInterface} implementation reports them by
 * contract.
 *
...
final readonly class CostCalculator
{
    ...
    public function costForCall(
        ...
        $cacheReadCost = ($cacheReadTokens / self::TOKENS_PER_MILLION) * $this->pricingProvider->cacheReadPricePerMillionTokens($model);
        $cacheCreationCost = ($cacheCreationTokens / self::TOKENS_PER_MILLION) * $this->pricingProvider->cacheCreationPricePerMillionTokens($model);
